...
export const PieChart = ({
  ...
  const customTooltipFormatter = (params: any) => {
    ...
    for (let i = 0; i < length; i++) {
      ...
      value = series?.[i]?.data?.[dataIndex].value * (isHalf ? 2 : 1)
      ...
      formatter:
        length > 1
          ...
          : (params: any) => {
              ...
              const value = params.value * (isHalf ? 2 : 1)
              const percent = params.percent * (isHalf ? 2 : 1)
              return ` ${params.marker} ${name} <br/>${value} (${percent}%)`
            },
      position: function (pos: any, params: any, el: any, elRect: any, size: any) {
        const obj: any = { marginTop: "1px" }
        obj[["left", "right"][+(pos[0] < size.viewSize[0] / 4)]] = 0
        return obj
      }
    },
    emphasis: {
